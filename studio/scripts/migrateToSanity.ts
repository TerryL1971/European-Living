...
async function migratePhraseCategories() {
  ...
  try {
    const { data: categories, error } = await supabase.from('phrase_categories').select('*').order('sort_order', { ascending: true });
    ...
    console.error('❌ Failed migrating phrase categories:', err);
  }
...
async function migratePhrases() {
  ...
  try {
    ...
    console.error('❌ Failed migrating phrases:', err);
  }
}
...
async function migrateArticles() {
  ...
  try {
    ...
    console.error('❌ Failed migrating articles:', err);
  }
...
async function migrateDestinations() {
  ...
  try {
    ...
    if (error) throw error;
    if (!destinations?.length) return console.log('No destinations found');
    ...
    console.error('❌ Failed migrating destinations:', err);
  }
